...
public class AStarState
{
    ...
    // Определение всех открытых точек и их местоположение.
    private Map<Location, Waypoint> open_waypoints
    = new HashMap<Location, Waypoint> ();
    
    // Определение всех закрытых точек и их местоположение.
    ...
    public Waypoint getMinOpenWaypoint()
    {
        ...
        return null;
        // Определяем наборы всех ключей открытых точек
        // итератор для итерации по набору
        // и переменную для хранение лучшей путевой точки.
        ...
        while (i.hasNext())
        {
            ...
            // чем сохраненная стоимость для сохраненной точки то,
            // заменить сохраненную точку текущей,
        ...
        // Возвращает точку с минимальной общей стоимостью.
        return best;
    }
...
// Находит местоположение новой точки.
        ...
        {
            // Если в новой точке уже есть открытая  точка
            // проверяет, меньше ли значение «предыдущей стоимости» новой  точки
            ...
            if (newWP.getPreviousCost() < current_waypoint.getPreviousCost())
            {
                ...
                // новая точка заменяет старую и возвращает true.
                open_waypoints.put(location, newWP);
                return true;
            }
            // Если значение «предыдущей стоимости» новой точки не меньше,
            // чем «предыдущая стоимость» текущей точки, вервращает false.
            return false;
        }
        // Если в новой точке еще нет открытой точки местоположениz
        // добавляет новую точку в коллекцию открытых точек
        // и возвращает true. 
        open_waypoints.put(location, newWP);
        return true;
    }
